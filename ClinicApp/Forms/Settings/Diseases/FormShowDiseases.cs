...
namespace ClinicApp.Forms.Settings.Diseases
{
    public partial class FormShowDiseases : Form
    {
        public FormShowDiseases()
        {
            InitializeComponent();
        }
        ...
        private void FormShowDiseases_Load(object sender, EventArgs e)
        {
            loadTable("select * from Diseases");
        ...
        void search(string text = null)
        {
            ...
            {
                loadTable("select * from Diseases where name like '%" + text + "%'");
            }
        }
            ...
            if (dgvLoading.Rows.Count > 0)
            {
                if (MessageBox.Show("هل تريد الحذف", "?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        cmd = new SqlCommand("delete from Diseases Where id = '" + id + "'", adoClass.sqlcn);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                ...
                {
                    DataRow dro = tbl.Tables["dtShowDiseases"].NewRow();
                    dro["name"] = dgvLoading[1, i].Value;
                    dro["notes"] = dgvLoading[0, i].Value;

                    tbl.Tables["dtShowDiseases"].Rows.Add(dro);
                }

                FormReports rptForm = new FormReports();
                rptForm.mainReport.LocalReport.ReportEmbeddedResource = "ClinicApp.Reports.ReportFormShowDiseases.rdlc";
                rptForm.mainReport.LocalReport.DataSources.Clear();
                rptForm.mainReport.LocalReport.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowDiseases"]));

                if (bool.Parse(declarations.systemOptions["directPrint"].ToString()))
                {
                    LocalReport report = new LocalReport();
                    string path = Application.StartupPath + @"\Reports\ReportFormShowDiseases.rdlc";
                    report.ReportPath = path;
                    report.DataSources.Clear();
                    report.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowDiseases"]));
