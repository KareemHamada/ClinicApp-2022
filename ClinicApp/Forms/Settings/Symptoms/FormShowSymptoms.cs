...
{
    public partial class FormShowSymptoms : Form
    {
        public FormShowSymptoms()
        {
            InitializeComponent();
        }
        ...
        private void FormShowSymptoms_Load(object sender, EventArgs e)
        {
            loadTable("select * from Symptoms");
        ...
        void search(string text = null)
        {
            ...
            {
                loadTable("select * from Symptoms where name like '%" + text + "%'");
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                if (MessageBox.Show("هل تريد الحذف", "?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (id == "")
                    {
                        MessageBox.Show("حدد العرض المراد حذفه");
                        return;
                    }
                    try
                    {

                        cmd = new SqlCommand("delete from Symptoms Where id = '" + id + "'", adoClass.sqlcn);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                ...
                {
                    DataRow dro = tbl.Tables["dtShowSymptoms"].NewRow();
                    dro["name"] = dgvLoading[1, i].Value;
                    dro["notes"] = dgvLoading[0, i].Value;

                    tbl.Tables["dtShowSymptoms"].Rows.Add(dro);
                }

                FormReports rptForm = new FormReports();
                rptForm.mainReport.LocalReport.ReportEmbeddedResource = "ClinicApp.Reports.ReportFormShowSymptoms.rdlc";
                rptForm.mainReport.LocalReport.DataSources.Clear();
                rptForm.mainReport.LocalReport.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowSymptoms"]));

                if (bool.Parse(declarations.systemOptions["directPrint"].ToString()))
                {
                    LocalReport report = new LocalReport();
                    string path = Application.StartupPath + @"\Reports\ReportFormShowSymptoms.rdlc";
                    report.ReportPath = path;
                    report.DataSources.Clear();
                    report.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowSymptoms"]));
