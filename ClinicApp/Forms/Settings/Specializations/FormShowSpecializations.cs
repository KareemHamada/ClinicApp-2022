...
{
    public partial class FormShowSpecializations : Form
    {
        public FormShowSpecializations()
        {
            InitializeComponent();
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                if (MessageBox.Show("هل تريد الحذف", "?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (id == "")
                    {
                        MessageBox.Show("حدد التخصص المراد حذفه");
                        return;
                    }
                    try
                    {

                        cmd = new SqlCommand("delete from Specializations Where id = '" + id + "'", adoClass.sqlcn);
                    ...
                    loadTable("select * from Specializations");
        ...
        void search(string text = null)
        {
            ...
            {
                loadTable("select * from Specializations where name like '%" + text + "%'");
            }
        }

        private void FormShowSpecializations_Load(object sender, EventArgs e)
        {
            loadTable("select * from Specializations");
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                ...
                {
                    DataRow dro = tbl.Tables["dtShowSpecializations"].NewRow();
                    dro["name"] = dgvLoading[0, i].Value;

                    tbl.Tables["dtShowSpecializations"].Rows.Add(dro);
                }

                FormReports rptForm = new FormReports();
                rptForm.mainReport.LocalReport.ReportEmbeddedResource = "ClinicApp.Reports.ReportFormShowSpecializations.rdlc";
                rptForm.mainReport.LocalReport.DataSources.Clear();
                rptForm.mainReport.LocalReport.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowSpecializations"]));

                if (bool.Parse(declarations.systemOptions["directPrint"].ToString()))
                {
                    LocalReport report = new LocalReport();
                    string path = Application.StartupPath + @"\Reports\ReportFormShowSpecializations.rdlc";
                    report.ReportPath = path;
                    report.DataSources.Clear();
                    report.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowSpecializations"]));
