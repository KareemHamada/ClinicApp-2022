...
{
    public partial class FormShowAnalysis : Form
    {
        public FormShowAnalysis()
        {
            InitializeComponent();
        }
        ...
        private void FormShowAnalysis_Load(object sender, EventArgs e)
        {
            loadTable("select * from Analysis");
        ...
        void search(string text = null)
        {
            ...
            {
                loadTable("select * from Analysis where name like '%" + text + "%'");
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                if (MessageBox.Show("هل تريد الحذف", "?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (id == "")
                    {
                        MessageBox.Show("حدد التحليل المراد حذفه");
                        return;
                    }
                    try
                    {

                        cmd = new SqlCommand("delete from Analysis Where id = '" + id + "'", adoClass.sqlcn);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                ...
                rptForm.mainReport.LocalReport.ReportEmbeddedResource = "ClinicApp.Reports.ReportFormShowAnalysis.rdlc";
                ...
                if (bool.Parse(declarations.systemOptions["directPrint"].ToString()))
                {
                    ...
                    string path = Application.StartupPath + @"\Reports\ReportFormShowAnalysis.rdlc";
