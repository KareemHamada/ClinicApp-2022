...
{
    public partial class FormShowIncomesTypes : Form
    {
        public FormShowIncomesTypes()
        {
            InitializeComponent();
        }
        ...
        private void FormShowIncomesTypes_Load(object sender, EventArgs e)
        {
            loadTable("select * from VisitingType");
        ...
        void search(string text = null)
        {
            ...
            {
                loadTable("select * from VisitingType where name like '%" + text + "%'");
            }
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                if (MessageBox.Show("هل تريد الحذف", "?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (id == "")
                    {
                        MessageBox.Show("حدد نوع الايراد المراد حذفه");
                        return;
                    }
                    try
                    {

                        cmd = new SqlCommand("delete from VisitingType Where id = '" + id + "'", adoClass.sqlcn);
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvLoading.Rows.Count > 0)
            {
                ...
                {
                    DataRow dro = tbl.Tables["dtShowIncomesTypes"].NewRow();
                    dro["notes"] = dgvLoading[0, i].Value;
                    dro["name"] = dgvLoading[1, i].Value;
                    tbl.Tables["dtShowIncomesTypes"].Rows.Add(dro);
                }

                FormReports rptForm = new FormReports();
                rptForm.mainReport.LocalReport.ReportEmbeddedResource = "ClinicApp.Reports.ReportFormShowIncomesTypes.rdlc";
                rptForm.mainReport.LocalReport.DataSources.Clear();
                rptForm.mainReport.LocalReport.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowIncomesTypes"]));

                if (bool.Parse(declarations.systemOptions["directPrint"].ToString()))
                {
                    LocalReport report = new LocalReport();
                    string path = Application.StartupPath + @"\Reports\ReportFormShowIncomesTypes.rdlc";
                    report.ReportPath = path;
                    report.DataSources.Clear();
                    report.DataSources.Add(new ReportDataSource("DataSet1", tbl.Tables["dtShowIncomesTypes"]));
